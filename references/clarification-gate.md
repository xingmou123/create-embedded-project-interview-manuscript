...
除非用户在具体项目中明确覆盖，直接采用以下默认值，不重复询问：

- 项目处于小批量量产阶段；
- 最终三个调试案例都是真实发生、已有实证的问题；
- 输入材料中已经出现的量化结果都来自真实场景。

这些默认值只确认成熟度、案例性质和已有结果的真实性，不授权虚构命名客户、认证证书、项目日期、团队规模或个人职责。
...
`global_defaults` 必须保留上述三个固定值。具体项目的例外写入 `explicit_overrides`，每项记录 `field`、`value` 和 `source_ref`；同一字段只能覆盖一次。成熟度可覆盖为 `prototype`、`engineering_validation`、`small_batch` 或 `mass_production`；案例性质可覆盖为 `confirmed_real`、`fault_injection` 或 `design_inference`；已有量化结果可覆盖为 `confirmed_real` 或 `unverified`。存在 `pending` 或 `unresolved`、用户已被询问但没有对应决定、确认稿尚未确认、全局默认缺失或覆盖没有明确来源时，禁止继续正式交付。
